def get_values(*names):
    import json
    _all_values = json.loads("""{"num_samples":96,"deepwell_type":"nest_96_wellplate_2ml_deep","res_type":"nest_12_reservoir_15ml","starting_vol":560,"binding_buffer_vol":420,"wash1_vol":600,"wash2_vol":600,"wash3_vol":600,"elution_vol":200,"lyse_vol":200,"mix_reps":15,"settling_time":5,"park_tips":false,"tip_track":false,"flash":false}""")
...
def run(ctx):
    ...
    plate_shake = ctx.load_labware(deepwell_type, '3')
...
' + str(settling_time) + ' minutes.')
    ...
    def lyse(vol, park=True):
        # add lysis buffer
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            _pick_up(m300)
            side = 1 if i % 2 == 0 else -1
            loc = m.bottom(0.5).move(Point(x=side*2))
            m300.aspirate(vol, lysis[i//3])
            m300.move_to(m.center())
            m300.dispense(210, loc)
            m300.blow_out(m.bottom(5))
            m300.air_gap(20)
            if park:
                m300.drop_tip(spot)
