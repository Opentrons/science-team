def get_values(*names):
    import json
    _all_values = json.loads("""{"num_samples":8,"deepwell_type":"nest_96_wellplate_2ml_deep","res_type":"nest_12_reservoir_15ml","starting_vol":400,"binding_buffer_vol":430,"wash1_vol":500,"wash2_vol":500,"wash3_vol":500,"elution_vol":50,"mix_reps":15,"settling_time":7,"park_tips":false,"tip_track":false,"flash":true}""")
...
def run(ctx):
    ...
    wash3 = res2.wells()[5:8]
    elution_solution = res2.wells()[-1]
    wash4 = res2.wells()[8:11]
...
resuming.')
    ...
    def bind(vol, park=False):
        ...
        for i, (well, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            for t in range(num_trans):
                ...
                if chan_ind > latest_chan:  # mix if accessing new channel
                    bead_mixing(source, m300, 200, reps=6) #total reps (aspirate and dispense) = reps*2
                    latest_chan = chan_ind
                m300.transfer(vol_per_trans, source, well.top(), air_gap=20,
                              new_tip='never')
                if t < num_trans - 1:
                    m300.air_gap(20)
            bead_mixing(well, m300, 200, reps=6) #total reps (aspirate and dispense) = reps*2
        ...
        ctx.pause('Move plate to Heater/ Shaker off-deck')
    ...
    def wash(vol, source, mix_reps=15, park=False, resuspend=True):
        """
        ...
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            for n in range(num_trans):
                ...
                if n < num_trans - 1:  # only air_gap if going back to source
                    m300.air_gap(20)
            if resuspend:
                resuspend_pellet(m, m300, 200, reps=4) #reps*4 = total reps
            m300.blow_out(m.top())
            m300.air_gap(20)
            if park:
                m300.drop_tip(spot)
        ...
        ctx.pause('Mix off deck on heater shaker for 2 minutes')
    ...
    def dnase(vol, source, mix_reps=6, park=False, resuspend=True):
        """
        dnase adds the DNAseI onto the plate
        """
        ...
        ctx.pause('Mix off deck on heater shaker for 5 minutes')

    def stop_reaction(vol, source, mix_reps=6, park=False, resuspend=True):
        ...
        ctx.pause('Mix off deck on heater shaker for 10 minutes')

        if magdeck.status == 'disengaged':
            magdeck.engage(height=MAG_HEIGHT)

        for stopi in np.arange(settling_time,0,-0.5):
            ctx.delay(minutes=0.5, msg='There are ' + str(stopi) + ' minutes left in the incubation.')

        remove_supernatant(vol, park=park)

    def elute(vol, park=False):
        """
        ...
        ctx.pause('Allow elution binding for 4 minutes off deck on heater/shaker')
    ...
    #wash(300, wash4, park=park_tips) RNA guide only has 3 washes- only 1 with EtOH
    #dnase1 treatment
